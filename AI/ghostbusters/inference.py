...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghost_position in self.allPositions:
            prob_obs_given_position = self.getObservationProb(observation, pacman_position, ghost_position, jail_position)
            self.beliefs[ghost_position] *= prob_obs_given_position
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, prob in newPosDist.items():
                new_beliefs[newPos] += self.beliefs[oldPos] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for ind in range(self.numParticles):
            pos_index = ind % len(self.legalPositions)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            beliefs[particle] += self.getObservationProb(observation, pacman_pos, particle, jail_pos)

        weight_sum = beliefs.total()

        if weight_sum:
            new_particles = [beliefs.sample() for _ in range(self.numParticles)]
