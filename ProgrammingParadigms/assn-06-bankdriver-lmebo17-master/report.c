...
Report_Transfer(Bank *bank, int workerNum, AccountNumber accountNum,
  ...
  sem_wait(&bank->report_transfer_guard);                
  ...
  if (amountAbs < reportingAmount) {
    sem_post(&bank->report_transfer_guard);
...
Report_DoReport(Bank *bank, int workerNum){
  sem_wait(&bank->checking_last);
  bank->busy_workers--;
  if(bank->busy_workers != 0){
    sem_post(&bank->checking_last);
    sem_wait(&bank->num_workers_new_day);
    sem_post(&bank->lock);
    return 0;
  } 
  Report *rpt = bank->report;
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      for(int i = 0; i < numWorkers - 1; i++){
        sem_post(&bank->num_workers_new_day);
      }
      for(int i = 0; i < numWorkers - 1; i++){
        sem_wait(&bank->lock);
      }
      bank->busy_workers = numWorkers;
      sem_post(&bank->checking_last);
      return -1;
  }
