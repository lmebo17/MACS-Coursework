...
Bank_Init(int numBranches, int numAccounts, AccountAmount initalAmount,
  ...
  sem_init(&bank->lock , 0 , 0);
  sem_init(&bank->checking_last , 0 , 1);
  sem_init(&bank->num_workers_new_day , 0 , 0);
  sem_init(&bank->report_transfer_guard , 0 , 1);
...
Bank_Balance(Bank *bank, AccountAmount *balance){
  ...
  for(int i = 0; i < bank->numberBranches; i++){
    sem_wait(&bank->branches[i].branch_lock);
  }
