...
(define (partition pivot num-list cmp_fn)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) cmp_fn)))
	(if (cmp_fn (car num-list) pivot)
...
(define (quicksort num-list cmp_fn)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) cmp_fn)))
	(append (quicksort (car split) cmp_fn) 
		(list (car num-list)) 
		(quicksort (cadr split) cmp_fn)))))
...
(define (intersection-points circles)
    (if ( < (length circles) 2) `()
      (append 
          (apply append (map (lambda(ls) (intersect (car circles) ls)) (cdr circles)))
          (intersection-points (cdr circles))
      )))


(define (count_products ls)
    (cond ((= (length ls) 0) 1)
          ((and (< (length ls) 2) (= (car ls) 0)) 1)
          ((< (length ls) 2) (car ls))
          ((= (car ls) 0) (count_products (cdr ls)))
    (else (* (car ls) (count_products (cdr ls))))))
          
              

(define (distance-product point point_ls)
    (count_products (map (lambda(x) (dist x point)) point_ls)))


(define (rate-points point_ls)
    (map (lambda(x) (append (list (distance-product x (reverse point_ls))) (list x))) point_ls))


(define (cmp_fn ls1 ls2)
    (< (car ls1) (car ls2))
)
...
(define (clumped-points point_ls)
      (map (lambda(x) (cadr x))
            (prefix-of-list (sort-points (rate-points point_ls)) (quotient (length point_ls) 2))
      )
)


(define (count_x point_ls)
    (if (null? point_ls) 0
        (+ (car(car point_ls)) (count_x (cdr point_ls)))))


(define (count_y point_ls)
    (if (null? point_ls) 0
        (+ (car(cdr (car point_ls))) (count_y (cdr point_ls)))))



(define (average-point point_ls)
    (let ((avarage (append (list(/ (count_x (reverse point_ls)) (length point_ls))) (list(/ (count_y (reverse point_ls)) (length point_ls))))))
        (list (distance-product avarage (reverse point_ls)) avarage))
)

(define (best-estimate point_ls)
    (average-point (clumped-points (intersection-points point_ls))))

(define (where-am-i distances starss)
    (sort-points (map (lambda(x) (best-estimate x)) (all-guesses distances starss))))
